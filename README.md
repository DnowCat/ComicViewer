...
| 步骤 | 说明 |
|------|------|
| 1 | 将下载输出目录（或你整理后的父目录）作为本阅读器的 **书架根目录**（`DEFAULT_SHELF_ROOT` 或启动参数，见下文）。 |
| 2 | 若下载结果尚未包含元数据 JSON，请在每部漫画文件夹内补充 `info.json`（或 `meta.json` / `book.json` / `comic.json`）及封面图，以便书架展示书名、标签与封面（见 [目录约定](#目录约定)）。 |
| 3 | 启动本程序：`python main.py [书架根目录]` |
